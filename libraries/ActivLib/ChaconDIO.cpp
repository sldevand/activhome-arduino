...
void ChaconDIO::setParameters(uint8_t p_pinTx, uint8_t p_pinRx,
                              unsigned long p_sender) {
  pinTx = p_pinTx;
  pinRx = p_pinRx;
  m_sender = p_sender;
...
void ChaconDIO::go(int interruptor, byte onoff) {
  ...
  itob(m_sender,
       26); // conversion du code de l'emetteur (ici 8217034) en code binaire
...
// Calcul le nombre 2^chiffre indiqué, fonction utilisée par itob pour la
...
// Convertit un nombre en binaire, nécessite le nombre, et le nombre de bits
// souhaités en sortie (ici 26)
...
// Envoi d'une paire de pulsation radio qui definissent 1 bit réel : 0 =01 et 1
// =10 c'est le codage Manchester, il permet entre autres de dissocier les
// données des parasites
...
void ChaconDIO::transmit(int blnOn) {
  ...
  delayMicroseconds(275 *
                    coeffDelay); // un bit de bruit avant de commencer pour
                                 // remettre les delais du recepteur a 0
  digitalWrite(pinTx, LOW);
  delayMicroseconds(9900 * coeffDelay); // premier verrou de 9900µs*/
  digitalWrite(pinTx, HIGH);            // high again
  delayMicroseconds(275 *
                    coeffDelay); // attente de 275µs entre les deux verrous
  digitalWrite(pinTx, LOW);      // second verrou de 2675µs
  delayMicroseconds(2675 * coeffDelay);
  digitalWrite(
      pinTx,
      HIGH); // On reviens en état haut pour bien couper les verrous des données
  ...
  // Envoie des 4 derniers bits, qui représentent le code interrupteur, ici 0
  // (encode sur 4 bit donc 0000) nb: sur  les télécommandes officielle chacon,
  // les interrupteurs sont logiquement nommés de 0 à x interrupteur 1 = 0 (donc
  // 0000) , interrupteur 2 = 1 (1000) , interrupteur 3 = 2 (0100) etc...
  for (i = 0; i < 4; i++) {
  ...
  digitalWrite(
      pinTx, LOW); // verrou 2 de 2675µs pour signaler la fermeture du signal
  ...
  if (t > coeffDelayRcv * 4000 && t < coeffDelayRcv * 11000) {
    ...
    if (t > coeffDelayRcv * 40 && t < coeffDelayRcv * 4000) {
      ...
      while (i < 64) {
        ...
        if (i % 2 == 1) {
          if ((prevBit ^ bit) ==
              0) { // must be either 01 or 10, cannot be 00 or 11
            i = 0;
            break;
          }
...
unsigned long ChaconDIO::getPulse() { return pulseIn(pinRx, LOW, 100000); }
